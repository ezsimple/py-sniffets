...
async def run(playwright: Playwright) -> None:
    ...
    # 요소가 로드될 때까지 기다리기
    await page.wait_for_selector('ul.listTagsTwoColumn')

    # 선택자로 링크 요소들 가져오기
    links = await page.query_selector_all('ul.listTagsTwoColumn > li > a.gr-hyperlink')

    # 각 링크에서 href 속성 추출
    href_list = [await link.get_attribute('href') for link in links if await link.get_attribute('href')]

    MAX_ATTEMPTS = 5
    for href in href_list:
        URL = BASE_URL + href
        for no in range(1, 101):
            URL = f"{BASE_URL}{href}?page={no}"
            tag = href.split('/')[-1]
            ...
            for q in quotes:
                ...
                with open(f'quotes-{tag}.csv', 'a', encoding='utf-8') as f:
