...
class AutoJobkorea:
    ...
    def __init__(self):
        ...
        self.configFile = "jobkorea_config.ini"
        self.readConfig()
        self.logging = LogUtil("JOBKOREA_ACCESS")
        ...
        # 파이썬 3 항연산자
        # self.chat_id = (
        #    self.telegram_chat_id if len(updates) == 0 else updates[-1].message.chat_id
        # )
        self.chat_id = ""
        self.writeConfig()
    ...
    def http_post(self, url, data):
        response = r.post(url, data=data, headers=self.headers)
        return response
    ...
    def readConfig(self):
        try:
            ...
            self.pw = self.config.get("CONFIG", "PW")
    ...
    def writeConfig(self):
        ...
        if self.chat_id == self.telegram_chat_id:
            return
        self.config.add_section("CONFIG")
        self.config.set("CONFIG", "HOST", self.host)
        self.config.set("CONFIG", "ID", self.id)
        self.config.set("CONFIG", "PW", self.pw)
        self.config.set("CONFIG", "KEY", self.key)
    ...
    def run(self):
        ...
        LOGIN_URL = self.host + "/Login/Logout.asp"
        ...
        with r.Session() as session:
            ...
            # 2. 로그인 하기
            LOGIN_INFO = {}
            LOGIN_INFO["M_ID"] = self.id
            LOGIN_INFO["M_PWD"] = self.pw
            req = session.post(
                LOGIN_URL, headers=self.headers, data=LOGIN_INFO, verify=False
            )
            if not req.ok:
                reason = str(req.status_code) + " " + req.reason
            ...
            # 3. 이력서 업데이트 페이지로 이동
            # PAGE = self.host + "/User/ResumeMng"
            # req = session.get(PAGE)
            # if not req.ok:
            #     reason = str(req.status_code) + " " + req.reason
            #     msg = reason + " #ERROR# 이력서 업데이트 페이지로 이동 실패!!"
            #     error(msg)
            #     # self.telegram_bot(msg)
            #     return
            ...
            # 5. 오늘 날짜로 업데이트 클릭
            PAGE = self.host + "/User/Resume/SetEditDate"
